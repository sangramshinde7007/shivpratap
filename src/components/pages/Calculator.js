...
export default function FDCalculator() {
  ...
  const maturityAmount =
    amount && years && roi
      ? amount * Math.pow(1 + roi / 100, years)
      : 0;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-rose-50 to-red-50 py-12 px-4 sm:px-6 lg:px-8 flex flex-col items-center">
      ...
      <div className="text-center mb-10 transform hover:scale-105 transition-transform duration-300">
        ...
        <p className="text-gray-600 text-lg">Calculate your returns with our premium rates</p>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-pink-100 hover:shadow-2xl transition-shadow duration-300">
          <div className="bg-gradient-to-r from-pink-600 to-rose-500 p-6 text-white">
            <h2 className="text-2xl font-bold flex items-center">
              <FaCalculator className="mr-3" /> Calculate Returns
            </h2>
          </div>
          ...
          <div className="p-8 space-y-8">
            ...
            <div className="group">
              <label className="block text-sm font-semibold text-gray-700 mb-2 flex items-center">
                <FaMoneyBillWave className="mr-2 text-pink-500" /> Deposit Amount (₹)
              </label>
            ...
            <div className="group">
              <label className="block text-sm font-semibold text-gray-700 mb-2 flex items-center">
                <FaClock className="mr-2 text-pink-500" /> Period (Years)
              </label>
              <div className="relative">
                 <input
                  type="number"
                ...
                />
                <span className="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none text-gray-500 font-medium">Years</span>
              </div>
            </div>
        ...
        {amount > 0 ? (
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-pink-100 hover:shadow-2xl transition-shadow duration-300 h-full flex flex-col">
            <div className="bg-gradient-to-r from-gray-800 to-gray-900 p-6 text-white">
               <h2 className="text-2xl font-bold flex items-center">
                <FaChartPie className="mr-3 text-pink-400" /> Maturity Summary
              </h2>
            </div>
            ...
            <div className="p-8 flex-1 flex flex-col justify-center space-y-6">
              ...
              <div className="bg-gradient-to-br from-pink-50 to-rose-50 p-6 rounded-xl border border-pink-100 text-center transform hover:scale-105 transition-transform duration-300">
                <p className="text-gray-600 font-semibold mb-2">Total Maturity Amount</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div className="p-4 rounded-xl border border-gray-100 bg-gray-50 hover:bg-white hover:shadow-md transition-all">
                    <p className="text-sm text-gray-500 mb-1">Invested Amount</p>
                    <p className="text-xl font-bold text-gray-800">₹{parseFloat(amount).toLocaleString('en-IN')}</p>
                 </div>
                 <div className="p-4 rounded-xl border border-green-100 bg-green-50 hover:bg-white hover:shadow-md transition-all">
                    <p className="text-sm text-green-600 mb-1">Total Interest</p>
                    <p className="text-xl font-bold text-green-700">₹{interestEarned.toLocaleString('en-IN', { maximumFractionDigits: 2 })}</p>
                 </div>
              </div>
              ...
              <div className="mt-4 pt-6 border-t border-gray-100">
                ...
                <div className="w-full bg-gray-200 rounded-full h-4 overflow-hidden flex">
                  <div 
                    ...
                    style={{ width: `${(amount / maturityAmount) * 100}%` }}
                  ></div>
                  <div 
                ...
                <div className="flex justify-between text-xs text-gray-500 mt-2">
                   <span>{((amount / maturityAmount) * 100).toFixed(1)}%</span>
                   <span>{((interestEarned / maturityAmount) * 100).toFixed(1)}%</span>
                </div>
              </div>
          ...
          <div className="hidden lg:flex h-full items-center justify-center bg-white/50 rounded-2xl border-2 border-dashed border-gray-200 text-gray-400 p-8">
             <div className="text-center">
                ...
                <p className="text-xl font-medium">Enter details to confirm maturity</p>
             </div>
          </div>
        )}
      </div>
